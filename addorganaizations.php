...
if(isset($_POST)) {
	...
	if($result->num_rows == 0) {
		...
		if($uploadOk == false) {
			header("Location: register-organaizations.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO organizations(name, companyname, country, state, city, contactno, website, email, password, aboutme, logo) VALUES ('$name', '$companyname', '$country', '$state', '$city', '$contactno', '$website', '$email', '$password', '$aboutme', '$file')";

		if($conn->query($sql)===TRUE) {

			//If data inserted successfully then Set some session variables for easy reference and redirect to company login
			$_SESSION['registerCompleted'] = true;
			header("Location: login-organaizations.php");
			exit();
